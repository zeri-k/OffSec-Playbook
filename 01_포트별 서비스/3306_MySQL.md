...
| 2 | 원격 DB 인증 | `mysql -h <TARGET> -u <USER> -p` | 로그인 성공과 `user@host` 거부를 구분한다. |
| 3 | TLS 오류 분리 | `mysql -h <TARGET> -u <USER> -p --ssl-mode=DISABLED` | 현재 client의 TLS 문제를 확인하고, 구형 client에서는 `--ssl=0`을 검토한다. |
| 4 | DB 목록 | `show databases;` | 현재 계정으로 접근 가능한 DB를 확인한다. |
| 5 | 권한과 파일 경로 | `show variables like 'secure_file_priv';`, `show grants;` | FILE·SUPER·GRANT 등 권한과 import/export 제한 경로를 연결한다. |
