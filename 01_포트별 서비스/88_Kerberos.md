...
**출력 해석 경계:** TCP/UDP 88 응답은 KDC 후보로의 도달성만 확정하며 realm 설정·유효 계정·취약점은 확정하지 않는다. `valid user`는 사용자명 존재 단서이고 비밀번호 인증이나 권한을 증명하지 않는다. `$krb5asrep$`/`$krb5tgs$`와 TGT/TGS는 offline cracking 또는 서비스 인증에 사용할 자료를 확정하지만, 평문 비밀번호·관리자 권한·SMB/LDAP/WinRM 접근 성공은 실제 서비스 요청으로 별도 검증한다.
...
| TGT/TGS 또는 `.kirbi` | [[Pass the Ticket]] | `klist`, `rubeus`, `impacket-ticketConverter` | ticket 주체 권한의 서비스 인증 |
