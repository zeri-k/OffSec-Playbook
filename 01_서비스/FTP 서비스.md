...
FTP 로그인과 명령 응답은 제어 채널에서 오가지만 `LIST`·`RETR`·`STOR`의 실제 목록·파일은 별도 데이터 채널로 전송된다. Active 모드에서는 서버가 클라이언트가 지정한 주소·포트로 연결하고, Passive 모드에서는 클라이언트가 서버가 알려 준 주소·포트로 연결한다. 따라서 `230` 로그인 성공 뒤 `ls`·`get`만 실패하면 자격 증명보다 데이터 채널 방향과 방화벽을 먼저 확인한다.
...
| 2 | 목록·읽기·쓰기 권한 | FTP 세션에서 `status`, `binary`, `ls`, `get`, `mget`, `put` | 현재 전송 모드와 데이터 채널 동작을 확인하고 익명/인증 사용자별 파일 읽기와 쓰기를 따로 구분한다. |
...
- 사용 중인 `ftp` 클라이언트가 지원할 때 `passive`로 데이터 연결 방향을 전환한 뒤 같은 `ls` 또는 `get`을 한 번 재시도한다. 명령 지원 여부와 현재 모드는 `help passive`, `status`로 확인한다.
