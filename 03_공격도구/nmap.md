...
```shell
sudo nmap <TARGET> -sn -PE --disable-arp-ping --packet-trace --reason
```

ARP를 끄고 ICMP Echo Request 응답을 직접 확인한다. `--packet-trace`와 `--reason`을 함께 쓰면 왜 host up으로 판단했는지 추적하기 좋다.
...
```shell
sudo nmap -sU --top-ports 20 <TARGET> -oA scans/udp-top
```

UDP는 느리고 응답이 모호하므로 처음에는 `--top-ports`로 자주 쓰는 포트부터 확인한다.

### HTTP 열거 스크립트

```shell
nmap -sV --script=http-enum -p 80,8080 <TARGET> -oA scans/http-enum
```

웹 서버의 흔한 경로, 애플리케이션 단서를 찾을 때 사용한다.

### SMB 열거 스크립트

```shell
nmap -p 445 --script smb-enum-shares,smb-enum-users <TARGET> -oA scans/smb-enum
```

SMB 공유, 사용자 등 기본 열거가 가능한지 확인한다. 인증이 필요한 스크립트는 별도 인자를 요구할 수 있다.

### 취약점 카테고리 스크립트

```shell
nmap -sV --script vuln -p 80,445 <TARGET> -oA scans/vuln
```

NSE의 `vuln` 카테고리를 실행한다. 결과는 단서로 보고, 실제 취약 여부는 버전/환경/검증 절차로 다시 확인한다.
...
SMTP 릴레이 허용 여부를 빠르게 확인한다. 스크립트 결과는 단서로 보고, 실제 릴레이 성공 여부와 허용되는 발신·수신 조합은 [[25_SMTP#Open Relay 검증|SMTP Open Relay 검증]]에서 구분한다.

### 방화벽/필터링 확인

```shell
sudo nmap -Pn -n --packet-trace -p 445 <TARGET>
```

필터링, 응답 없음, 패킷 흐름을 확인할 때 사용한다. 필요하면 `--reason`, `-sS`, `-sT`, `--source-port` 등을 비교한다.

### 결과 저장

```shell
nmap -sC -sV -p 22,80,445 <TARGET> -oA scans/target-service
```

`-oA`는 normal, grepable, XML 세 형식을 한 번에 저장하므로 이후 정리와 자동화에 편하다.
...
| 출력/상태 | 의미 | 다음 행동 |
|---|---|---|
| `open` | 해당 포트에서 서비스 응답 확인 | 포트별 서비스 노트로 이동해 열거/공격 후보 선택 |
