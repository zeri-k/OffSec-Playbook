...
### Kerberos ticket으로 관리자 share 확인

```bash
smbclient //<DC_HOST>/C$ -k -no-pass -c 'ls'
```

Kerberos 인증은 IP보다 FQDN을 쓰는 편이 안정적이다.

### share 안의 파일을 일괄 다운로드

```bash
smbclient //<INTERNAL_TARGET>/HR -U '<DOMAIN>\<USER>%<PASSWORD>' -c 'prompt OFF; mget *'
```

### 재귀 다운로드

```bash
smbclient //<TARGET>/notes -U 'user%<PASSWORD>' -c 'recurse ON; prompt OFF; mget *'
```

### SMB dialect 지정

```bash
smbclient //<TARGET>/share -U 'user%<PASSWORD>' -m SMB3
```
