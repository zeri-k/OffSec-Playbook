...
```bash
curl -X POST -d 'username=<USER>&password=<PASSWORD>' http://<TARGET>/login
```
...
| 파일 다운로드/업로드 성공 | 파일 전송 또는 기능 검증 가능 | 저장 파일 무결성, 업로드 위치, 실행 가능 여부 확인 |
