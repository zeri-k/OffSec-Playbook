...
`username-anarchy`는 실명을 여러 계정명 표기 규칙으로 변환해 사용자명 후보 목록을 만드는 도구다. 조직의 계정명 형식을 아직 확정하지 못했을 때 가능한 표기를 넓게 생성하는 데 유용하며, 출력 자체는 실제 계정 존재를 뜻하지 않는다.
...
성과 이름 또는 이름 목록을 입력하면 여러 규칙의 사용자명 후보를 출력한다.
