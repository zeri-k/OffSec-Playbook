...
### 자식 도메인 ExtraSids Golden TGT로 부모 계정 DCSync
...
- `<CHILD_USER>`는 ticket의 요청자이고 `<TARGET_USER>`는 부모 DC에서 추출할 대상이다.
- `-target-ip`는 부모 DC로 연결할 IP를 지정하지만 Kerberos 서비스 이름은 `<ROOT_DC_FQDN>`으로 유지한다.
- 구버전 Impacket의 cross-realm 처리에서는 `-dc-ip <ROOT_DC_IP>`가 첫 자식 realm 요청까지 부모 KDC로 고정해 `KDC_ERR_WRONG_REALM`을 만들 수 있다. 자식·부모 도메인이 DNS로 해석된다면 `-dc-ip`를 생략하고 debug 출력에서 양쪽 KDC 연결을 확인한다.

### 도메인 dump를 파일로 분리 저장

```bash
impacket-secretsdump -outputfile <PREFIX> -just-dc '<DOMAIN>/<USER>:<PASSWORD>@<DC>'
```
