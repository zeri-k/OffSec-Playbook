...
### 현재 사용 중인 계정이 보유한 도메인 객체 ACL 확인
...
- `User-Force-Change-Password`, `GenericWrite`, `GenericAll`, replication right 같은 사람이 읽을 수 있는 `ObjectAceType`

### 특정 AD 계정의 디렉터리 복제 권한 확인

```powershell
$sid = Convert-NameToSid '<DOMAIN>\<CONTROLLED_USER_OR_GROUP>'
Get-ObjectAcl '<DOMAIN_DN>' -ResolveGUIDs |
...
- 같은 `SecurityIdentifier`의 `DS-Replication-Get-Changes`와 `DS-Replication-Get-Changes-All`.
- 두 항목 모두 `AccessAllowed`, `ExtendedRight`이고 `ObjectDN`이 대상 도메인 루트여야 한다.
- 이 결과의 판정과 다음 실행은 [[AD 계정의 디렉터리 복제 권한 확인]]에서 이어 간다.
...
- `SourceName`, `TargetName`, `TrustAttributes`, `TrustDirection`.
- `WITHIN_FOREST`와 `FOREST_TRANSITIVE`를 구분하고 각 방향의 객체 조회·인증을 별도로 확인한다.

### 신뢰 대상 도메인의 SPN 사용자 확인

```powershell
Get-DomainUser -SPN -Domain <TARGET_FQDN> | Select-Object SamAccountName,MemberOf,ServicePrincipalName
```

확인할 출력:

- 대상 도메인의 `SamAccountName`과 `ServicePrincipalName`.
- `MemberOf`는 공격 우선순위 단서이며 비밀번호 복구나 실제 권한을 뜻하지 않는다.

### 신뢰 대상 도메인의 외부 그룹 멤버십 확인

```powershell
Get-DomainForeignGroupMember -Domain <TARGET_FQDN>
Convert-SidToName <FOREIGN_MEMBER_SID>
```

확인할 출력:

- 권한을 부여하는 `GroupDomain`·`GroupName`과 다른 도메인에서 들어온 `MemberName`·SID.
- 변환한 계정이 대상 그룹에 포함됐더라도 실제 원격 접근과 권한은 대상 서비스에서 확인한다.
...
| `Get-NetLocalGroupMember` | 원격 호스트 로컬 그룹 구성원 조회 | RDP·WinRM 접근 권한 후보 확인 |
...
| `MemberName`과 `MemberSID` | 직접·중첩 그룹 구성원 확인 | 실제 권한 상속과 제어 가능한 계정 확인 |
