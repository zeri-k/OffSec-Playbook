...
현재 로그온 세션에 Kerberos 티켓을 주입한다. 이후 `dir \\host\share` 같은 Windows 명령으로 접근 여부를 확인한다.

### Pass the Hash로 새 프로세스 생성
...
NTLM 해시를 사용해 지정한 사용자 컨텍스트의 새 프로세스를 만든다. 생성된 `cmd.exe`에서 네트워크 접근을 검증한다.
...
로컬 계정 해시와 LSA secret 확인에 사용한다. `token::elevate`는 SYSTEM 토큰으로 상승 가능한 상황에서 함께 사용한다.
...
도메인 컨트롤러 복제 권한을 가진 계정으로 도메인 계정 해시를 요청한다. `krbtgt` 해시는 Golden Ticket 검토에 쓰인다.

다른 도메인의 계정을 요청할 때는 대상 도메인을 명시한다.

```cmd
mimikatz # lsadump::dcsync /domain:<TARGET_FQDN> /user:<TARGET_NETBIOS>\<TARGET_USER>
```

### 부모 권한 SID를 포함한 Golden Ticket 생성·주입
...
`SID`는 자식 도메인 SID, `Extra SIDs`는 부모 `Enterprise Admins` SID와 일치해야 한다. 주입 성공 뒤 부모 DC 공유 접근이나 부모 도메인 DCSync로 실제 권한을 확인한다.

### Credential Manager/Vault 확인
...
Windows 자격 증명 관리자에 저장된 항목을 확인한다. DPAPI 보호 값은 별도 masterkey 복호화가 필요할 수 있다.
