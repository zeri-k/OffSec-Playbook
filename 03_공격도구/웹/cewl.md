...
`cewl`은 웹사이트를 크롤링해 페이지에 노출된 단어와 이메일 주소를 수집하고 대상 맞춤형 wordlist로 저장한다. 조직명·제품명·연도처럼 대상 문맥을 반영한 비밀번호 크래킹 후보나 사용자명 단서를 만들 때 유용하지만, 수집 항목 자체가 유효한 계정이나 비밀번호를 뜻하지는 않는다.
...
- 필요한 파일·값: 필요하면 크롤링 깊이, 최소 단어 길이, 숫자 포함 여부, wordlist와 이메일 출력 파일 경로
