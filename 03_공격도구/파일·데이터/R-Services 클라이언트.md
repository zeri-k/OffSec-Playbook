...
- 포트 512/513/514가 식별되면 [[R-Services 서비스]]에서 service별 노출을 먼저 구분하고, trust 조건이 보일 때만 [[R-Services trust 기반 원격 접근]]으로 이어간다.
- `nmap`으로 포트와 서비스 단서를 다시 확인할 때는 [[nmap]]의 service scan 결과를 이 문서의 실제 client 연결로 재검증한다.

## 참고 링크

- [GNU Inetutils manual](https://www.gnu.org/software/inetutils/manual/inetutils.html)
- [OpenBSD hosts.equiv and .rhosts format](https://man.openbsd.org/OpenBSD-5.4/hosts.equiv.5)
