...
```bash
./kerbrute_linux_amd64 passwordspray --safe --dc <TARGET> --domain <DOMAIN> users.txt '<PASSWORD>'
```
...
`--safe`는 KDC가 계정 잠금을 반환한 뒤 모든 thread를 중단한다. 실행 전에 계정별 잠금 정책이나 현재 실패 횟수를 조회하지 않으므로 “잠금 예방” 또는 실행 가능 판정으로 해석하지 않는다. 정책·계정 상태를 먼저 확인하고 첫 잠금·disabled 응답이나 첫 유효 credential에서 전체 실행을 중지한다.
...
- [Kerbrute 공식 저장소 — Usage, User Enumeration과 Password Spray](https://github.com/ropnop/kerbrute)
