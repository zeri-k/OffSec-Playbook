...
- `--headers`, `--sslinfo`, `--whois`, `--crawl`, `--dns`, `--sub`, `--dir`, `--wayback`, `--ps`, `--full` 같은 지원 옵션이 도움말에 표시되는지 확인한다.
- 도움말 출력은 로컬 설치가 실행된다는 뜻일 뿐, 대상 URL이나 외부 조회 서비스 도달성을 확정하지 않는다.
...
지원 모듈을 넓게 실행하면 출력이 많다. 필요한 모듈만 골라 실행하고 헤더/TLS/크롤링 단서 중심으로 본다.

확인할 출력:

- 각 모듈 섹션에 결과 또는 오류가 독립적으로 기록되는지 확인한다. 일부 섹션이 비었거나 실패했다면 나머지 섹션의 성공과 분리한다.
