...
결과: ["SYSTEM token 후보", "선택한 API로 생성한 SYSTEM 자식 프로세스"]
---
...
- 현재 token: `SeAssignPrimaryTokenPrivilege`와 일반적으로 `SeIncreaseQuotaPrivilege`, 또는 다른 모드에서는 `SeImpersonatePrivilege`
...
| `{<CLSID>};NT AUTHORITY\SYSTEM` | 인증 흐름에서 SYSTEM token 표시 | 선택한 process 생성 API 성공을 별도 확인 |
| `CreateProcessAsUser OK` | `-t u`의 자식 process 생성 성공 | 자식 명령의 실제 Identity 확인 |
| `CreateProcessAsUser Failed to create proc: 1314` | 필요한 privilege가 token에 없거나 API가 사용할 수 없음 | SeAssignPrimaryToken·SeIncreaseQuota 존재와 token access rights 확인 |
