...
결과: ["DNS server·zone 설정 조회", "승인된 DNS 설정·record 변경", "operation별 성공·오류 상태"]
---
...
- 변경 입력: exact parameter, 기존 값과 복구 값. DNS service 재시작 권한은 `dnscmd` 설정 권한과 별도다.
...
- 설정은 다음 DNS service start 때 사용할 path를 지정한다. 실제 DLL load와 code execution은 service 상태·proof로 따로 확인한다.
...
| `/config <PARAMETER>` | server-level 설정 변경 | exact 복구 값을 확보한 승인된 작업 |
...
| `/recordadd`·`/recorddelete` | DNS record 추가·삭제 | 기존 record와 exact 복구를 기록한 승인 작업 |
...
| `ERROR_ACCESS_DENIED`·`Status = 5` | 현재 token의 해당 operation 권한 부족 | 현재 token, 대상 server와 DnsAdmins·위임 범위 확인 |
