...
| Windows Meterpreter 세션에서 대상에 도구나 스크립트를 반입해야 하고 쓰기 가능한 경로가 있음 | [[Meterpreter upload로 Windows 파일 반입]] | Windows 대상의 파일과 무결성 확인 결과 | [[Windows 셸 또는 세션 확보 후 컨텍스트 열거]] | 활성 세션, 로컬 원본 경로, 대상 쓰기 경로와 송수신 SHA-256 확인 |
...
- 파일 전송 성공 메시지만 믿지 않고 송신본과 수신본의 크기와 SHA-256을 비교한다.
