...
|---|---|---|---|
| `<USER>`의 plaintext password | `-p '<PASSWORD>'` | `-p '<PASSWORD>'` | WinRM 인증 뒤 해당 계정의 PowerShell 세션 |
...
- 그룹 멤버십이나 BloodHound `CanPSRemote` edge는 실제 WinRM endpoint와 prompt를 보장하지 않는다.
