...
필요권한: ["대상 호스트의 SYSTEM 명령 실행 권한", "서비스 생성 방식은 로컬 Administrators 그룹과 상승된 token"]
필요조건: ["탈취할 RDP session ID", "현재 세션 이름", "tscon.exe 사용 가능"]
...
대상 Windows 호스트에서 SYSTEM으로 명령을 실행할 수 있고 다른 사용자의 RDP session ID가 확인되면 `tscon`으로 그 세션을 현재 RDP session에 연결하여 대상 사용자의 기존 desktop과 현재 권한을 확인한다.
...
| 실행 권한 | `tscon`이 SYSTEM으로 실행됨 | `whoami /all` 또는 SYSTEM 서비스 실행 | 일반 관리자 token과 SYSTEM을 구분 |
...
| `Access is denied` | `tscon` 실행 주체가 SYSTEM이 아니거나 session 제어 권한 부족 | 세션 전환 실패 | 현재 token과 서비스 실행 주체 확인 |
| 명령 성공처럼 보이나 desktop이 바뀌지 않음 | 잘못된 대상 ID·destination 또는 Windows 버전 제한 | 세션 전환 실패 | 현재 session name, 대상 ID와 운영체제 지원 여부 확인 |
