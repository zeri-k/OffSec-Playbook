...
- NFSv3 export가 보이거나 NFSv4 pseudo-root가 의심될 때.
- 홈 디렉터리·백업·웹·배포 경로에서 파일과 자격 증명 후보를 수집할 때.
- 클라이언트 사용자 이름이 아니라 서버가 평가하는 숫자 UID·GID를 확인할 때.
...
```bash
mkdir -p ./target-nfs
sudo mount -t nfs -o vers=3,ro,nosuid,nodev,nolock <TARGET>:/<EXPORT> ./target-nfs
findmnt --target ./target-nfs
```

### NFSv4 읽기 전용 마운트

```bash
sudo mount -t nfs4 -o ro,nosuid,nodev <TARGET>:/ ./target-nfs
findmnt --target ./target-nfs
```
...
```bash
find ./target-nfs -maxdepth 2 -mindepth 1 -printf '%M %u:%g %p\n'
sudo umount ./target-nfs
sudo mount -t nfs4 -o ro,nosuid,nodev <TARGET>:/<EXPORT> ./target-nfs
findmnt --target ./target-nfs
```
...
```bash
find ./target-nfs -maxdepth 3 -type f \( -name 'id_*' -o -name '*.bak' -o -name '*.conf' \) -print
ls -lan ./target-nfs
stat -c '%n %s bytes uid=%u gid=%g mode=%a' ./target-nfs/<FILE>
sha256sum ./target-nfs/<FILE>
sudo umount ./target-nfs
```
...
| 관찰 | 판단 | 결과 상태 | 다음 행동 |
|---|---|---|---|
| `ro` mount와 파일 목록 확인 | 현재 source에서 export 읽기 가능 | NFS 파일 접근 | 필요한 파일의 크기·hash·UID/GID 확인 |
| 파일 내용과 hash 확인 | 해당 파일 READ 권한 | 파일·자격 증명 후보 수집 | 파일 유형별 후속 인증·크래킹 선택 |
...
sudo umount ./target-nfs
findmnt --target ./target-nfs || echo 'NFS unmounted'
rmdir ./target-nfs
```
