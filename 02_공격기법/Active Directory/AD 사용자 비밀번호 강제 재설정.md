...
- 원래 비밀번호를 모르면 이번 변경 전 자격 증명으로 자동 복원할 수 없다. 승인된 복구 담당자와 새 관리 비밀번호 배포 절차가 없으면 실행하지 않는다.
...
복원 요청 성공과 실제 인증 성공을 분리한다. DC 복제 상태를 고려해 승인된 한 서비스에서 인증을 확인하고, 작업 전에 식별한 서비스·예약 작업·응용 프로그램의 상태를 담당자와 대조한다. 원래 문자열을 다시 설정해도 `pwdLastSet`, 비밀번호 기록, 감사 이벤트, 이미 영향을 받은 세션·티켓과 종속 시스템 상태까지 이전 시점으로 돌아가지는 않으므로 `원래 비밀번호 사용 가능 확인`으로 기록하고 완전 원복으로 표현하지 않는다.

### 원래 비밀번호를 모르는 경우

자동 원복은 불가능하다. 계정 소유자 또는 승인된 도메인 관리자가 새 관리 비밀번호를 설정하고, 종속 서비스·예약 작업·응용 프로그램의 저장 자격 증명을 갱신한 뒤 각각의 정상 동작을 확인해야 한다. 이 확인이 끝나기 전에는 `복구 완료`가 아니라 `관리자 복구 인계`로 기록한다. 새 비밀번호와 원래 비밀번호는 Vault에 남기지 않는다.
