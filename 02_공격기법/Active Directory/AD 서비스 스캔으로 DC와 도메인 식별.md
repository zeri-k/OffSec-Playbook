...
필요조건: ["대상 호스트 목록", "대상 TCP 서비스에 도달 가능한 네트워크 경로", "능동 스캔 범위와 출력 basename"]
...
> 능동 연결과 NSE 요청은 대상 로그와 탐지 시스템에 남으며 클라이언트에서 되돌릴 수 없다. 이 문서는 대상 설정을 바꾸지 않고, 생성한 로컬 결과 파일만 exact 경로로 정리한다.
...
| 스캔 범위와 영향 | host 목록과 AD 식별용 TCP 포트만 능동 확인 | 대상 수, 포트와 NSE 범위를 작업 전에 대조 | 산업·레거시 장비 등 불안정 가능 대상은 능동 스캔에서 제외 |
| 결과 저장 위치 | Vault 밖의 작업 디렉터리와 새 `<SCAN_PREFIX>` | `.nmap`·`.xml`·`.gnmap` 부재 확인 | 기존 결과를 덮지 않는 basename 사용 |
...
- 대상 수가 현재 확인한 호스트 manifest와 일치한다.
...
- `-sS` 권한 오류가 나면 root/raw socket 권한을 확인한다. raw socket을 쓸 수 없을 때만 `-sT` connect scan으로 바꾸고, 연결이 대상 로그에 남을 수 있음을 고려한다.
- `-sC`는 기본 NSE script를 실행하므로 대상 안정성·응답 부하와 필요한 결과를 확인한다. 단순 포트 상태만 필요하면 먼저 `-sC`를 빼고 확인한다.
- 원천의 `-A`는 OS 탐지·버전 탐지·기본 script·traceroute를 함께 켠다. 이 문서의 첫 선택에는 필요 범위를 넘으므로 사용하지 않고, 추가 프로파일링은 후보 하나와 필요한 옵션으로 제한한다.
...
결과를 폐기하기로 했다면 기록한 세 exact 경로만 제거하고 부재를 확인한다. 같은 디렉터리의 다른 scan 결과를 pattern으로 삭제하지 않는다.
