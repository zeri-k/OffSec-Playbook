...
필요권한: ["인증은 hash 주체의 대상 서비스 로그온 권한", "SMB·WMI·PsExec 원격 명령 실행은 대상 로컬 관리자 권한", "Restricted Admin 기반 RDP PtH는 대상 로컬 Administrators 권한"]
...
| GUI 세션 필요 | `<TARGET>:3389`, Restricted Admin Mode와 hash 주체의 대상 로컬 Administrators 권한 | xfreerdp `/pth` | RDP GUI 세션. 실제 token은 세션 안에서 별도 확인 |
...
- RDP PtH는 Restricted Admin Mode와 hash 주체의 대상 로컬 Administrators 권한을 별도 확인하고, GUI 내 `whoami /all`로 실제 token·무결성 수준을 확인한다. `CanRDP` edge나 `Remote Desktop Users` 멤버십만으로 Restricted Admin 조건을 대체하지 않는다.
...
| RDP 실패 | Restricted Admin Mode, 로컬 Administrators 권한, NTLM 허용 또는 RDP 경로 문제 | 시작 상태 유지 | 조건을 분리하고 WinRM/SMB/WMI로 대체 |
