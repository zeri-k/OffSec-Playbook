...
`%USERDOMAIN%`과 `%LOGONSERVER%`는 현재 로그온 문맥의 단서이며, DNS suffix·route·ARP 결과만으로 AD 서비스 또는 내부 대상 도달성을 확정하지 않는다.
...
- 이 문서의 기본 명령은 제한 셸에서 즉시 사용할 기준선이다. 더 많은 객체 속성과 Linux 원격 열거는 각 세부 기법 문서에서 수행한다.
