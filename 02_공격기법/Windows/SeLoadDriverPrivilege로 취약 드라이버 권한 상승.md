...
필요조건: ["대상 OS·architecture와 호환되는 취약한 signed kernel driver", "승인된 EoPLoadDriver와 driver별 exploit", "App Control·HVCI·vulnerable driver blocklist가 해당 driver load를 차단하지 않음"]
...
현재 token에 `SeLoadDriverPrivilege`가 있고 승인된 취약 driver가 대상 build·architecture와 호환되며 Code Integrity 정책이 load를 허용할 때만, privilege-aware loader로 driver를 등록·load하고 driver별 exploit가 생성한 새 process의 Identity로 SYSTEM 상승을 검증한다.
...
| driver 입력 | exact hash·signature·architecture가 확인된 승인된 취약 driver | `Get-FileHash`, `Get-AuthenticodeSignature` | 임의 driver나 출처 불명 binary를 load하지 않음 |
| loader·exploit | driver registry path를 만들고 privilege를 활성화·load하는 loader와 해당 driver 전용 exploit | 도구 hash와 upstream 사용법 확인 | generic loader 성공을 driver exploit 성공으로 확대하지 않음 |
...
| 영향 승인 | kernel crash·호스트 재부팅·보안 제품 탐지 가능성을 수용한 작업 창 | 범위·복구 담당자·재부팅 가능 여부 확인 | 승인 없으면 열거와 차단 상태 확인에서 중단 |
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath '<VULNERABLE_DRIVER_PATH>','<EOPLOADDRIVER_PATH>','<DRIVER_EXPLOIT_PATH>'
...
- 세 파일의 exact SHA-256, driver signature status와 기존 registry key·loaded driver 부재.
...
- `SeLoadDriverPrivilege Enabled`, registry path와 driver load 단계의 NTSTATUS.
- NTSTATUS 문자열만 성공으로 해석하지 말고 `driverquery`와 CodeIntegrity event에서 exact driver가 loaded 상태인지 확인한다.
...
Capcom.sys의 승인된 실습 범위에서는 driver가 실제 loaded 상태일 때만 전용 exploit를 실행한다.
...
| CodeIntegrity·WDAC·blocklist event | driver load가 정책으로 차단됨 | 현재 경로 실행 불가 | 방어 제어를 유지하고 다른 승인된 권한 상승 후보로 전환 |
| privilege 이름이 현재 token에 없음 | 그룹 상태 또는 사용자 권한 할당이 token에 반영되지 않음 | 전제 미충족 | 새 로그온과 현재 계정·그룹 확인. privilege를 임의 부여하지 않음 |
| bugcheck·service 불안정 | kernel exploit 영향 발생 | host 장애 | 작업을 중단하고 승인된 복구·재부팅 절차 수행 |

## 변경 영향과 복구

EoPLoadDriver가 만든 이번 사용자 hive의 exact key와 반입 파일만 제거한다. registry key 삭제가 이미 loaded된 kernel driver를 unload하지는 않는다. driver가 unload routine을 제공하지 않거나 loader가 unload를 지원하지 않으면 승인된 재부팅 전까지 loaded 상태가 남을 수 있으므로 완전 복구로 기록하지 않는다.
...
registry query가 key 부재를 반환하고 파일이 없어도 `driverquery`에 남아 있으면 driver는 아직 loaded 상태다. 임의 `sc delete`나 다른 driver 제거로 확대하지 말고 승인된 재부팅 또는 driver별 unload 절차와 호스트 정상성 검증을 수행한다. event log·EDR 기록과 crash 영향은 파일 삭제로 복구되지 않는다.
