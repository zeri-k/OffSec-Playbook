...
|---|---|---|---|
| 대상 저장 파일 | 실행 전 존재하지 않은 exact `C:\Windows\Temp\<REMOTE_FILE>`, 크기와 SHA-256 | 파일을 사용하는 process를 먼저 종료한 뒤 `rm C:\Windows\Temp\<REMOTE_FILE>` | `ls C:\Windows\Temp\<REMOTE_FILE>`가 파일을 반환하지 않음 |

Meterpreter session은 이 기법의 입력이므로 여기서 일괄 종료하지 않는다. 이 업로드 뒤 별도 기법이 만든 process·session이 있으면 그 문서의 종료 순서를 먼저 따른다. 기존 원격 파일이 있었거나 hash를 기록하지 않았다면 이번 파일로 구분할 수 없으므로 삭제하지 않는다.
