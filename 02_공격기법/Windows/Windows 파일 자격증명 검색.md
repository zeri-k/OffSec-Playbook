...
- 현재 보유 정보: RDP·WinRM·SMB shell의 현재 사용자와 읽을 수 있는 프로필·공유 경로를 알고 있다.
- 명령 실행 위치와 도달성: 대상 Windows 호스트의 `cmd` 또는 PowerShell에서 검색하거나, 파일 접근 경로를 통해 회수한 파일을 분석 호스트에서 읽을 수 있다.
- 현재 가능한 행동과 결과: 현재 세션 계정의 파일 읽기 범위 안에서 IT·admin·dev 역할 기반 경로와 CredMan·DPAPI 외 파일을 검색해 credential 후보·key·보호 파일을 수집한다.
