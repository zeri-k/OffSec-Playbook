...
Windows 대상에서 명령을 실행하고 공격 호스트의 HTTP 포트에 연결할 수 있다면, `certutil.exe`로 원본 파일을 쓰기 가능한 경로에 내려받고 HTTP 요청·대상 파일 생성으로 반입을 확인한다.
...
`<SERVE_DIRECTORY>`는 공격 호스트의 원본 파일이 있는 디렉터리(예: `./transfer`), `<SOURCE_FILE>`은 그 안의 파일명(예: `tool.exe`)이다. `<ATTACKER_IP>`와 `<PORT>`는 Windows 대상이 도달할 공격 호스트의 IPv4 주소와 HTTP 포트이며, `<DESTINATION_FILE>`은 대상 Windows의 새 절대 경로다.
...
| 관찰 | 판단 | 결과 상태 | 다음 행동 |
|---|---|---|---|
| HTTP GET과 파일 생성 | 파일 반입 완료 | Windows 대상의 도구 파일 | 원래 사용하려던 공격기법으로 돌아가 파일 실행 |
...
- HTTP GET은 요청 도달, `command completed successfully`는 다운로드 명령 완료를 각각 입증한다.
...
|---|---|---|---|
| 대상 저장 파일 | 실행 전 `if exist` 결과가 없음, `<DESTINATION_FILE>` | Windows 대상에서 `del "<DESTINATION_FILE>"` | `if exist "<DESTINATION_FILE>" echo FILE_STILL_EXISTS`가 출력되지 않음 |
