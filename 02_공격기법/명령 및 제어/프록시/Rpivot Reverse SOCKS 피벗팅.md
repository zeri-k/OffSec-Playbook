...
3. 공격 호스트의 ProxyChains를 `socks4 127.0.0.1 9050`으로 설정한다.
...
- 공격 호스트의 `<ATTACKER_IP>:9999`에서 client 연결을 대기하고 `127.0.0.1:9050`에서 SOCKS4 포트를 listen한다.
...
피벗 호스트가 공격 호스트의 rpivot server로 직접 연결하지 못하고 조직의 NTLM 인증 HTTP proxy만 사용할 수 있을 때 적용한다.
...
```bash
proxychains curl -I http://<INTERNAL_IP>:80
```
