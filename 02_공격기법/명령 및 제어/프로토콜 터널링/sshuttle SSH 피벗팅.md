...
필요조건: ["피벗 호스트용 SSH password 또는 private key", "피벗 호스트에서 <INTERNAL_CIDR> route와 목표 TCP 포트 확인", "공격 호스트에서 iptables 또는 nftables 기반 라우팅 설정 가능"]
...
- 지금 가능한 행동: `<INTERNAL_CIDR>`의 여러 TCP 호스트·포트를 일반 클라이언트로 반복 확인한다. UDP와 ICMP는 같은 범위로 간주하지 않는다.
...
| 필요한 파일·목록·주소 | sshuttle과 `<PIVOT_IP>`, `<INTERNAL_CIDR>`, `<INTERNAL_IP>`, `<PORT>` | 설치 상태와 route 범위를 비교 | 잘못된 CIDR과 중복 route 수정 |
...
| UDP/ICMP 기반 검증 필요 | sshuttle만으로 부족할 수 있음 | TCP 도구 중심으로 확인하거나 다른 피벗 검토 |
...
| UDP·ICMP 도구만 실패함 | sshuttle이 해당 트래픽을 처리하지 못할 수 있음 | TCP 라우팅만 확보 | TCP 기반 검증 또는 다른 피벗 기법 사용 |
