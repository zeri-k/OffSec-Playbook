...
필요조건: ["OpenSSH scp client", "대상 SSH·SFTP 지원"]
결과: ["SSH 연결로 반입하거나 회수한 파일"]
---
...
공격 호스트에서 대상 SSH 서비스에 인증할 수 있고 송신 파일을 읽으며 수신 경로에 쓸 수 있으면, `scp`로 한 방향의 파일을 복사한다. 전송 손상 진단 또는 정확한 사본 여부가 필요한 경우에만 원격 `ssh` 명령을 포함한 양쪽 hash를 비교한다.
...
| 필요한 파일·목록·주소 | 로컬·원격 source와 destination exact 경로, SSH port | 작업 전 목적지 부재와 source 경로 확인 | 기존 파일을 덮어쓰지 않을 새 목적지 선택 |
...
ssh -p <SSH_PORT> <USER>@<TARGET> "ls -l '<REMOTE_DESTINATION>'"
```

확인할 출력:

- `scp` 종료 상태가 0이고 원격의 정확한 경로에 파일이 생성됐는지 확인한다.
- progress meter의 `100%`는 송신 종료를 나타내지만 원격 파일의 실행 권한까지 입증하지 않는다. 손상 진단이나 정확한 사본 비교가 필요할 때만 양쪽 hash를 대조한다.
...
ssh -p <SSH_PORT> <USER>@<TARGET> "test -r '<REMOTE_SOURCE_FILE>'"
...
- `scp` 종료 상태가 0이고 로컬의 정확한 경로에 회수본이 생성됐는지 확인한다. 정확한 사본 비교가 필요할 때만 원격 원본과 로컬 회수본 hash를 대조한다.
...
| 관찰 | 판단 | 결과 상태 | 다음 행동 |
|---|---|---|---|
| 인증 성공, 전송 종료 상태 0, 선택한 정확한 경로에 파일 생성 | 선택한 방향의 파일 복사 완료 | 파일 반입 또는 회수 | 파일 사용 조건 또는 분석 범위를 별도 확인 |
...
| 선택 비교에서 hash 불일치 | 다른 source·destination, 중단 또는 변형 가능 | 손상된 파일 | 파일을 사용하지 말고 exact 경로와 크기를 대조해 새 목적지로 재전송 |
...
|---|---|---|---|
| 원격 반입 파일 | 전송 전 `test ! -e`, exact 경로 | 파일을 사용하는 원격 process를 먼저 종료한 뒤 `ssh -p <SSH_PORT> <USER>@<TARGET> "rm -- '<REMOTE_DESTINATION>'; test ! -e '<REMOTE_DESTINATION>'"` | 원격 `test` 종료 상태 0 |
| 로컬 회수본 | 회수 전 `test ! -e`, exact 경로 | 분석 완료와 보존 여부 결정 뒤 `rm -- '<LOCAL_DESTINATION>'` | `test ! -e '<LOCAL_DESTINATION>'` 성공 |

작업 전 존재했거나 정확한 생성 경로를 기록하지 않은 파일은 이 절차로 안전하게 원상복구할 수 없다. SSH 연결이 끊겨 원격 파일 부재를 확인할 수 없으면 원격 정리 완료로 기록하지 않는다.
