...
- 현재 보유 정보: [[내부망 수동 호스트 식별]]에서 얻은 대상 CIDR 또는 내부 주소 단서가 있다.
- 명령 실행 위치와 도달 대상: 내부망 인터페이스가 있는 호스트나 ICMP를 실제로 전달하는 피벗 뒤에서 대상 CIDR로 route가 잡혀 있다. TCP 전용 프록시 경로는 이 조건을 충족하지 않을 수 있다.
- 현재 계정과 권한: 현재 OS 계정이 `fping`, `ping` 또는 `Test-Connection`을 실행하거나 현재 Meterpreter 세션이 `ping_sweep` 모듈을 사용할 수 있다. 대상 계정이나 서비스 자격 증명은 필요하지 않다.
- 지금 가능한 행동과 결과: 많은 IP에서 ICMP 응답 호스트를 빠르게 추려 서비스 열거 대상을 줄일 수 있지만, 운영체제·역할·열린 포트·취약성은 얻지 못한다.
...
- `<TARGET_PREFIX>`에는 `172.16.5`처럼 마지막 옥텟을 제외한 `/24` 접두사를 넣는다. 다른 prefix 길이에 이 반복 범위를 그대로 사용하지 않는다.
