...
| 도메인 객체에 복제 관련 권한 | DCSync 가능성 | 도메인 자격 증명 복제 후보 | [[AD 계정의 디렉터리 복제 권한 확인]]에서 같은 SID의 두 필수 권한을 확인하고, 충족하면 [[DCSync]] |
...
- `AccessAllowed`, 대상 `ObjectDN`, 현재 계정의 `SecurityIdentifier`와 권한 이름이 함께 확인되어야 한다.
