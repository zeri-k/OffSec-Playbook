...
| 관찰 | 판단 | 결과 상태 | 다음 행동 |
|---|---|---|---|
| 현재 계정이 Administrators 구성원이지만 토큰이 elevated 상태가 아님 | 관리자 Identity와 현재 프로세스 권한이 다름 | 제한된 관리자 토큰 후보 | 현재 세션의 무결성 수준과 승인된 상승 경로를 확인 |
| 현재 토큰에 흥미로운 privilege가 표시됨 | privilege 기반 권한 상승 전제 조건 후보 | 토큰 권한 단서 | privilege의 활성 상태, 대상 서비스·객체와 해당 원자 기법의 조건을 직접 확인 |
