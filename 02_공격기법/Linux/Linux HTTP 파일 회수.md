...
sha256sum '<UPLOAD_RECEIVE_DIRECTORY>/<RECEIVED_FILE_1>' '<UPLOAD_RECEIVE_DIRECTORY>/<RECEIVED_FILE_2>'
```
...
1. Linux 대상의 `curl`이 끝나고 수신 파일의 크기·SHA-256 비교가 완료됐는지 확인한다.
...
3. 승인된 분석·결과 인계가 끝난 뒤 이번 실행의 `<UPLOAD_RECEIVE_DIRECTORY>` 안에서 확인한 수신 파일만 `rm -- '<UPLOAD_RECEIVE_DIRECTORY>/<RECEIVED_FILE_1>' '<UPLOAD_RECEIVE_DIRECTORY>/<RECEIVED_FILE_2>'`로 제거한다.
