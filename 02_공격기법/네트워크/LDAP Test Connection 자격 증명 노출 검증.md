...
tags:
  - 서비스/ldap
시작조건: ["승인된 장비 관리 화면에서 LDAP 연결 설정과 Test Connection 기능 확인", "현재 LDAP 전송 보호 방식 확인"]
...
프린터·애플리케이션 관리 화면의 LDAP `Test Connection`이 TLS 없는 simple bind를 사용하는지 승인된 상태에서 확인할 수 있다면, 기존 설정을 보존한 채 서버 주소만 통제된 listener로 잠시 전환해 bind 전송을 관찰하고 즉시 원래 서버로 복원한다.
...
| 기존 상태 | server, port, LDAP/LDAPS·StartTLS, 인증 방식, Base DN과 인증서 검증 값 기록 | 승인된 작업 기록에 화면·값 보존, 비밀번호 원문은 기록하지 않음 | 기존 값을 모르면 변경하지 않음 |
| 적용 범위 | `Test Connection` 한 번과 서버 주소·port만 변경 | 저장이 즉시 운영 인증에 적용되는지 장비 문서·관리자 확인 | 운영 로그인·주소록 등에 영향이 있으면 별도 창 확보 |
...
검증 호스트에서 기존 listener 충돌과 pcap 파일 부재를 확인한다. 실제 bind 값은 Vault가 아닌 승인된 증적 위치에서만 취급한다.
...
- root가 필요한 낮은 port 대신 장비가 허용하는 고유 high port를 우선 사용한다. 장비가 port 변경을 지원하지 않으면 기존 389 listener 충돌과 승인 범위를 먼저 해결한다.
...
그 뒤 기록한 exact listener·capture PID만 종료하고 pcap을 승인된 절차에 따라 인계하거나 제거한다.
