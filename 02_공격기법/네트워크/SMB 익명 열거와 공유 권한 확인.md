...
`<TARGET>`은 SMB 서버 주소(가상 예시 `192.0.2.45`)이고, `<DOMAIN>`·`<USER>`·`<PASSWORD>`는 Null·Guest와 비교할 보유 계정의 인증 형식이다. `<SHARE>`는 목록 출력에서 선택한 공유이며, 아래 명령은 공격 호스트에서 각 인증 컨텍스트를 섞지 않고 실행해 목록·접근·READ 결과를 분리한다.
