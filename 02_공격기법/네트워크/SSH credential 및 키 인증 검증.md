...
`<USER>@<TARGET>`은 SSH 인증 대상(가상 예시 `<USER>@192.0.2.22`)이고, `<KEY_PATH>`는 공격 호스트의 private key 파일 경로다. `<KNOWN_HOSTS_FILE>`은 새 작업용 host-key 저장 경로이며, password·key는 SSH session용 자료일 뿐 다른 내부 서비스의 인증 자료가 아니다. 아래 명령은 SSH client를 실행하는 공격 호스트에서 같은 대상·계정을 재사용한다.
