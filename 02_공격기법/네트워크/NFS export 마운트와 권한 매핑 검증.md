...
- proof 삭제가 거부되면 export를 끊기 전에 정확한 `$MOUNTPOINT/$PROOF`와 숫자 UID/GID를 확인하고, 같은 identity 또는 승인된 서버 관리 경로로 제거한다. 확인할 수 없으면 원격 파일 정리 완료로 기록하지 않는다.
