...
test ! -e '<SSH_KNOWN_HOSTS_FILE>'
install -m 600 /dev/null '<SSH_KNOWN_HOSTS_FILE>'
```
...
ssh -M -S '<SSH_CONTROL_SOCKET>' -o UserKnownHostsFile='<SSH_KNOWN_HOSTS_FILE>' -o StrictHostKeyChecking=ask -o ExitOnForwardFailure=yes -fN -L 127.0.0.1:<LOCAL_PORT>:<INTERNAL_IP>:<INTERNAL_PORT> <USER>@<PIVOT_IP>
ssh -S '<SSH_CONTROL_SOCKET>' -O check <USER>@<PIVOT_IP>
nmap -sV -p<LOCAL_PORT> 127.0.0.1
```

확인할 출력:

- 공격 호스트의 `127.0.0.1:<LOCAL_PORT>`에서 `<INTERNAL_IP>:<INTERNAL_PORT>`의 서비스 배너가 보이면 로컬 포트 포워딩 성공이다. 이는 최종 서비스 인증 성공이나 권한 획득을 의미하지 않는다.

### SOCKS 동적 포워딩

```bash
ss -ltnp 'sport = :<SOCKS_PORT>'
ssh -M -S '<SSH_CONTROL_SOCKET>' -o UserKnownHostsFile='<SSH_KNOWN_HOSTS_FILE>' -o StrictHostKeyChecking=ask -o ExitOnForwardFailure=yes -fN -D 127.0.0.1:<SOCKS_PORT> <USER>@<PIVOT_IP>
ssh -S '<SSH_CONTROL_SOCKET>' -O check <USER>@<PIVOT_IP>
```
...
전역 ProxyChains 설정을 수정하지 않고 위 내용을 기존에 없는 `<SSH_PROXYCHAINS_CONFIG>`에 저장해 다음 명령의 `-f`와 같은 경로를 사용한다.

```bash
test ! -e '<SSH_PROXYCHAINS_CONFIG>'
printf 'strict_chain\nproxy_dns\n\n[ProxyList]\nsocks4 127.0.0.1 <SOCKS_PORT>\n' > '<SSH_PROXYCHAINS_CONFIG>'
sed -n '1,20p' '<SSH_PROXYCHAINS_CONFIG>'
```

```bash
proxychains -f '<SSH_PROXYCHAINS_CONFIG>' nc -vz <INTERNAL_IP> 3389
proxychains -f '<SSH_PROXYCHAINS_CONFIG>' nmap -sT -Pn -n -p3389 <INTERNAL_IP>
```

확인할 출력:

- ProxyChains 로그에 `127.0.0.1:<SOCKS_PORT> ... <INTERNAL_IP>:<PORT> ... OK`가 보인다.
...
```bash
proxychains -f '<SSH_PROXYCHAINS_CONFIG>' nc -vz <INTERNAL_IP> <PORT>
proxychains -f '<SSH_PROXYCHAINS_CONFIG>' nmap --unprivileged -sT -Pn -n --reason -p<PORT> <INTERNAL_IP>
```
...
```bash
ssh -o UserKnownHostsFile='<SSH_KNOWN_HOSTS_FILE>' -o StrictHostKeyChecking=ask <USER>@<PIVOT_IP> "ss -ltnp 'sport = :<PIVOT_LISTEN_PORT>'"
ss -ltnp 'sport = :<ATTACKER_LISTEN_PORT>'
ssh -M -S '<SSH_CONTROL_SOCKET>' -o UserKnownHostsFile='<SSH_KNOWN_HOSTS_FILE>' -o StrictHostKeyChecking=ask -o ExitOnForwardFailure=yes -fN -R <PIVOT_BIND_IP>:<PIVOT_LISTEN_PORT>:127.0.0.1:<ATTACKER_LISTEN_PORT> <USER>@<PIVOT_IP>
ssh -S '<SSH_CONTROL_SOCKET>' -O check <USER>@<PIVOT_IP>
```

확인할 출력:

- 첫 `ss`에서 피벗의 `<PIVOT_LISTEN_PORT>`는 기존 listener가 없어야 하고 공격 호스트의 `<ATTACKER_LISTEN_PORT>`에는 의도한 기존 수신기가 있어야 한다. 이후 피벗 호스트의 `<PIVOT_BIND_IP>:<PIVOT_LISTEN_PORT>`로 들어온 연결이 공격 호스트의 `127.0.0.1:<ATTACKER_LISTEN_PORT>` listener로 전달된다. 양쪽 `ss`와 공격 호스트 listener 도착을 각각 확인하고, 실제 payload session과 그 Identity를 별도로 확인한다. 원격 bind가 loopback 밖에서 필요하면 SSH 서버의 `GatewayPorts` 정책도 충족해야 한다.
...
3. 공격 호스트에서 `ssh -S '<SSH_CONTROL_SOCKET>' -O exit <USER>@<PIVOT_IP>`로 exact master를 종료한다. 이후 `ssh -S '<SSH_CONTROL_SOCKET>' -O check <USER>@<PIVOT_IP>`가 master 부재를 반환하고 해당 listener가 작업 전 상태로 돌아왔는지 확인한다.
