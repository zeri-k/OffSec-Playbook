...
기존 SPN이 있는 대상에는 `-Set`으로 전체 값을 덮어쓰지 않는다. 기존 값을 보존하면서 고유 값 하나만 추가하고, 복구 때 같은 값만 제거할 수 있는 LDAP 변경 방법을 사용한다.
...
- 기존 SPN이 있었다면 전체 `-Clear`를 사용하지 말고 이번에 추가한 `<UNIQUE_TEMP_SPN>`만 정확히 제거한다.
